...
echo "<tr align='center' style='height:40px; background-color:#5AA4A3; color:#FFFFFF;'>
		<td><a href='".$path.'calendar/'.$id."/".$prev_month."' style='color:#FFFFFF;'><i class='material-icons'>keyboard_arrow_left</i></a></td>
		<td align='center' colspan=5>$title $year</td>
		<td><a href='".$path.'calendar/'.$id."/".$next_month."' style='color:#FFFFFF;'><i class='material-icons'>keyboard_arrow_right</i></a></td>
	  </tr>";
echo "<tr align='center' style='height:40px; background-color:#EEEEEE; color:#5AA4A3;'>
		<td width='15%'>Sun</td>
		<td width='14%'>Mon</td>
		<td width='14%'>Tue</td>
		<td width='14%'>Wed</td>
...
while ( $day_num <= $days_in_month )
{
		...
		$ognoo_2022 = '2022-'.$month.'-'.str_pad($day_num, 2, "0", STR_PAD_LEFT);
		...
		echo "<td><div class='num'>";
				...
				if(strlen($irsen)>0)
				{
				    ...
				    else
				    {
				        if($today>=$ognoo)
    				    {
    				        echo "<a data-bs-toggle='modal' data-bs-target='#addteacher".$day_num."'>edit</a>";
    				    }
				    }
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
	  ...
	  <div class="modal-body">
		<table class="table table-bordered">
			...
			while($row1 = mysqli_fetch_array($query1))
			{
				...
				<td><select name="irts[]" class="form-control">';?>
      ...
      <div class="modal-footer">
		...
		<input type="submit" name="update_irts" class="btn btn-primary" value="Хадгалах">
      </div>
	</div>
  </div>
</div>
...
<?php
			...
			Ирц ".$data1['niit']."(".substr((100),0,4)."%)</span></div>";
				}
				else
				{
				    /*********************/
				    ...
				    else
				    {
				        if($today>=$ognoo)
				        {
                            ?><a data-toggle='modal' data-target='#add<?=$day_num?>' href='#'>--------<i class='material-icons md-48'>add_circle_outline</i></a><?php
                        }
}
?>
<!-- Modal add -->
...
<div class="modal fade" id="add<?=$day_num?>" tabindex="-1" role="dialog" aria-labelledby="addlevel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-default" data-dismiss="modal">Гарах</button>
		<input type="submit" name="insert_irts" class="btn btn-primary" value="Хадгалах">
      </div>
	</div>
  </div>
</div>
