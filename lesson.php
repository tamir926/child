            ...
            <section class="scroll-section" id="areaChartTitle">
              <div class="card">
                <div class="body">
                  ...
                  $branch = 3;
                  $login = 6;
                    if($login==5 || $login==2 || $login==3 || $login==4 || $login==6)
                    {
                      $branch = '';
                    }
                    else
                   ...
                   // echo "select *from sector where statuscode>0 $branch order by branch";
                  $query = mysqli_query($conn,"select *from sector where statuscode>0 $branch order by branch") or die (mysqli_error());
                  while($row = mysqli_fetch_array($query))
                  {
                    $main_teacher = $row['main_teacher'];
                    $sub_teacher = $row['sub_teacher'];
                    $data1 = mysqli_query($conn,"select a.lname , a.fname, a.brchno , b.name from employer a, branch b where a.id = $main_teacher and a.brchno = b.id order by b.id") or die (mysqli_error());
                    ...
                    if(!empty($data1['name']))
                    {
                      if(empty($data2['lname'])) { $lname2 = ''; } else { $lname2 = $data2['lname'];}
                      if(empty($data2['fname'])) { $fname2 = ''; } else { $fname2 = $data2['fname'];}
                    echo '<tr>
                          <td>'.$i.'</td>
                          <td>'.$data1['name'].'</td>
                          <td>'.$row['name'].'</td>
